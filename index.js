...
function getAppStateFromEnv() {
  const rawAppState = process.env.APPSTATE;
  if (!rawAppState) return null;

  try {
    return typeof rawAppState === "string"
      ? JSON.parse(rawAppState)
      : rawAppState;
  } catch (parseError) {
    console.error(
      "[MongoDB] Failed to parse APPSTATE from environment variable:",
      parseError,
    );
    return null;
  }
...
async function clearAppStateFromDb() {
  if (!MONGODB_URI) return;

  try {
    await AppStateModel.deleteMany({});
  } catch (mongoError) {
    console.error("[MongoDB] Failed to clear invalid appstate:", mongoError);
  }
}
...
(async () => {
  ...
  if (loginAppState) {
    try {
      api = await loginWithAppState(loginAppState);
      console.log("[MongoDB] Login via database appstate succeeded.");
    } catch (err) {
      await clearAppStateFromDb();
      console.warn(
        "[MongoDB] Invalid session detected. Clearing database and falling back to environment variable.",
      );
      loginAppState = getAppStateFromEnv();
      if (!loginAppState) {
        console.warn(
          "[MongoDB] No APPSTATE found in environment after clearing invalid database state.",
        );
      }
    }
  }
  ...
  if (!api) {
    ...
    try {
      ...
      return console.error("❌ লগইন ব্যর্থ:", err.error || err);
    }
  }
  ...
  if (MONGODB_URI) {
    setInterval(
      () => {
        saveAppStateToDb(api.getAppState()).catch((saveError) =>
          console.error("[MongoDB] Periodic appstate save failed:", saveError),
        );
      },
      5 * 60 * 1000,
    );
  ...
  api.listenMqtt(async (err, message) => {
    ...
    try {
      ...
      if (
        BAD_WORDS.some((word) => messageBodyLower.includes(word.toLowerCase()))
      ) {
        console.log(
